...
import { buildConfigObject, buildEnvEntries, buildInitOutput } from '@/bin/init/output.js'
import type { InitAnswers } from '@/bin/init/plan.js'
import type { ProvisionResult } from '@/bin/init/provision.js'
...
describe('generated config passes the plugin validator', () => {
  const cases: Array<Partial<InitAnswers>> = [
    ...
    { service: 'stream' },
    { signedUrls: true },
    { region: 'ny', service: 'storage', storageAccess: 'http' },
    { clientUploads: true, service: 'storage', storageAccess: 's3' },
    { clientUploads: true, deployEdge: true, service: 'storage', storageAccess: 'http' },
    { clientUploads: true, purge: true, service: 'both', signedUrls: true, storageAccess: 's3' },
    { optimizer: true },
    { optimizer: true, signedUrls: true },
  ]
