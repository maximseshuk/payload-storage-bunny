...
With `disablePayloadAccessControl: true`, each document's URL is generated from its stored `prefix` and signed. Each URL is a time-limited token bound to that exact file — a token minted for tenant A's file cannot fetch tenant B's — and files stream straight from the CDN edge. See [Signed URLs](/configuration/signed-urls).
...
:::danger Unsigned public URLs do not isolate tenants
...
Bunny Stream videos are library-scoped GUIDs; there are no per-tenant folders, so the prefix recipe does not apply to Stream. Isolate Stream two ways: keep Payload access control on (collection read rules gate the video — no prefix is involved, so unlike Storage this works), or enable signed URLs (`stream.tokenSecurityKey`), which scope the token to the individual video. Gate uploads with `stream.tus.checkAccess(req, body)` against your own tenant model.

To keep each tenant's videos tidy in the Bunny dashboard, you can drop them into a per-tenant Bunny **collection** — see [Stream collections](/stream-collections). Note that collections are organization only and do **not** isolate tenants; the two options above are what enforce isolation.
