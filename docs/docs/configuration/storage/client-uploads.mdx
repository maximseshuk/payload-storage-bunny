...
- **[`storage.s3`](/configuration/storage#s3-mode) is set → presigned S3.** The browser `PUT`s straight to Bunny's S3 endpoint. No Edge Script is involved, and `clientUploads.edge` is not applicable.
- **`storage.s3` is not set → edge.** The upload is proxied through a Bunny Edge Script, so `clientUploads.edge` is **required**.
...
When the zone sets `storage.s3`, the `clientUploads` object accepts only `access` and `prefix` — the `edge` block is not available for S3 zones. This is enforced by the type: a storage zone is either HTTP API (needs `edge`) or S3 (presigns directly).
...
Without `storage.s3`, the file is proxied through a [Bunny Edge Script](https://docs.bunny.net/docs/edge-scripting?ref=fndfoymy0j) that you deploy once. `clientUploads.edge` is required.
...
The server mints a signed HMAC URL for each upload: it appends an `X-Upload-Expires` timestamp, an `X-Upload-Zone` name, and an HMAC-SHA256 signature over the request. The Edge Script rejects anything whose signature doesn't verify against its `SHARED_SECRET`, so `edge.secret` must match the secret the script was deployed with.
