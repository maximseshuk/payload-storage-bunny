...
- Any collection using Payload's access control (the default — `disablePayloadAccessControl` is not `true`) and Stream needs either `mp4Fallback: true` or signed URLs with `staticHandler.useRedirect: true`. See [Stream](/configuration/stream#access-control-and-mp4-fallback).
