...
:::warning Collections organize, they don't isolate
Collections are **not** access control. Bunny doesn't make them public, you can't generate a URL for a collection, and a video's playback is governed by its library, GUID, and token — never by which collection it sits in. For real tenant isolation use [signed URLs](/configuration/signed-urls) or Payload access control — see [Multi-tenant](/multi-tenant#stream-has-no-folders).
:::
