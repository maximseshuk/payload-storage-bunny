...
Bunny Stream can organize videos into **collections** — folders inside a video library. It is a dashboard-organization feature: you can, for example, drop each tenant's videos into their own collection. Collections are managed through the Bunny Stream API; this plugin does not create or assign them for you, but everything you need to do it yourself is already on the document.

:::warning Collections organize, they do not isolate
Collections are **not** access control. Bunny does not make them public, you cannot generate a URL for a collection, and a video's playback is governed by its library, GUID, and token — never by which collection it sits in. For real tenant isolation use [signed URLs](/configuration/signed-urls) or Payload access control — see [Multi-tenant](/multi-tenant#stream-has-no-folders).
:::
...
- **Cache collection lookups.** Creating a collection is not idempotent — without a cache or an existence check you will spawn duplicates and hit the API on every upload. The module-level `Map` above is the minimum.
- **Bulk moves.** The dashboard cannot move many videos at once, but the same Update Video call looped over `payload.find(...)` results will.
